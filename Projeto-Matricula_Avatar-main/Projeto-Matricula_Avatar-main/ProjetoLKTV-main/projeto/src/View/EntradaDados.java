...
public class EntradaDados extends javax.swing.JFrame {
    ...
    private void carregarDeficiencias() {
        ...
        txtdeficiencia.addItem(new Deficiencia(0, "Nenhuma"));
        try {
            DeficienciaDAO dao = new DeficienciaDAO();
            for (Deficiencia d : dao.read()) {
                txtdeficiencia.addItem(d);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Erro ao carregar tipos de deficiência: " + e.getMessage());
        }
    }
    ...
    private void btnsalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnsalvarActionPerformed
        ...
        try {
            ...
            //Deficiencia
            Deficiencia def = (Deficiencia) txtdeficiencia.getSelectedItem();
            if (rbtnsimdef.isSelected() && def != null && def.getId() != 0) {
                aluno.setTipo_deficiencia(def.getId());
            } else {
                aluno.setTipo_deficiencia(0);
            }
    ...
    private javax.swing.JComboBox<objeto> txtdeficiencia;
